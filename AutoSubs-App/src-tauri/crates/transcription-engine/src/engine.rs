...
use eyre::eyre;
use crate::types::{SpeechSegment, DiarizeOptions, LabeledProgressFn, NewSegmentFn, Segment, ProgressType};
...
impl Engine {
    ...
    pub async fn transcribe_audio(
        ...
        println!("[ENGINE] 📖 Đọc WAV: {}", audio_path);
        // Gửi log về frontend
        if let Some(ref progress_cb) = cb.progress {
            progress_cb(0, ProgressType::Transcribe, "📖 Đọc file audio...");
        }
        let original_samples = crate::audio::read_wav(&audio_path)?;
        println!("[ENGINE] ✅ WAV OK: {} samples ({:.1}s audio), mất {:.2}s",
            original_samples.len(),
            original_samples.len() as f64 / 16000.0,
            t_wav.elapsed().as_secs_f64()
        );
        if let Some(ref progress_cb) = cb.progress {
            progress_cb(0, ProgressType::Transcribe, &format!("✅ Audio OK: {:.0}s", original_samples.len() as f64 / 16000.0));
        }
        ...
        if let Some(true) = options.enable_diarize {
            ...
            println!("[ENGINE] 🔍 VAD: phát hiện giọng nói...");
            let vad_model_path_str = vad_model_path.to_string_lossy().to_string();
            // Truyền progress callback vào VAD để emit log chi tiết từng bước
            speech_segments = crate::vad::get_segments(&vad_model_path_str, &original_samples, cb.progress)
                .map_err(|e| eyre!("{:?}", e))?;
            println!("[ENGINE] ✅ VAD OK: {} segments, mất {:.2}s",
                speech_segments.len(),
                t_vad.elapsed().as_secs_f64()
            );
        }
        else {
        ...
        // === LOG: Thông tin segments trước khi transcribe ===
        println!("[ENGINE] 📊 Transcribing {} segments, total {} samples ({:.1}s audio)",
            speech_segments.len(),
            num_samples,
            num_samples as f64 / 16000.0
        );
        // In chi tiết từng segment
        for (i, seg) in speech_segments.iter().enumerate() {
            println!("[ENGINE]   segment[{}]: {:.2}s - {:.2}s ({} samples)",
                i, seg.start, seg.end, seg.samples.len()
            );
        }
        ...
        // === LOG: Bắt đầu transcription engine ===
        let t_transcribe = Instant::now();
        println!("[ENGINE] 🚀 Engine: {} | model_path: {:?}",
            if is_parakeet { "PARAKEET" } else if is_moonshine { "MOONSHINE" } else { "WHISPER" },
            _model_path
        );

        let (mut segments, detected_lang) = if is_parakeet {
            // Use Parakeet engine
            println!("[ENGINE] 🦜 Gọi transcribe_parakeet()...");
            let result = crate::engines::parakeet::transcribe_parakeet(
                _model_path.as_path(),
                speech_segments,
                &options,
                cb.progress,
            ...
            .await?;
            println!("[ENGINE] ✅ Parakeet xong sau {:.2}s", t_transcribe.elapsed().as_secs_f64());
            result
