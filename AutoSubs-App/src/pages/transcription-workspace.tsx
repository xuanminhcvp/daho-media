...
export const TranscriptionWorkspace = () => {
    ...
    // Handle export Whisper Words (word-level timestamps)
    // Đọc originalSegments từ transcript file → extract word-level → cho user Save As
    const handleExportWhisperWords = async () => {
        try {
            // Lấy filename transcript hiện tại
            ...
            // Đọc transcript file để lấy originalSegments (chứa word-level timestamps)
            const transcript = await readTranscript(filename);
            if (!transcript) {
                console.error("[WhisperWords] Không tìm thấy transcript file:", filename);
                return;
            }

            // originalSegments chứa word-level data gốc từ Whisper
            const originalSegments = transcript.originalSegments || transcript.segments || [];

            // Gọi util export → hiện dialog Save As cho user chọn folder
            const success = await exportWhisperWordsFile(originalSegments);
            if (success) {
                console.log("[WhisperWords] ✅ Export thành công!");
            }
        } catch (error) {
            console.error("[WhisperWords] ❌ Export failed:", error);
        }
    };
