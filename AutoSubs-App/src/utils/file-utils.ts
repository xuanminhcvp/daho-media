...
import { getTranscriptsDirPath } from '@/services/auto-media-storage';

// Get the transcripts storage directory
// Trỏ sang ~/Desktop/Auto_media/data/transcripts/
export async function getTranscriptsDir(): Promise<string> {
  const dir = await getTranscriptsDirPath();
...
function interpolateWordsFromText(text: string, start: number, end: number) {
  ...
  const wordTexts = safeText.match(/(\s*\S+)/g) || [];
